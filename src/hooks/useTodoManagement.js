  ...
  switch (action.type) {
    ...
    case 'SET_FILTER':
      ...
      return {
        ...
        showCompleted: filter === 'completed' ? true : (state.currentFilter === 'completed' && filter !== 'completed') ? false : state.showCompleted
      }

    case 'SET_CATEGORY':
      const categoryData = state.categories.find(cat => cat.id === action.payload.category)
      return {
        ...state,
        ...
        showCompleted: false,
        currentIcon: categoryData ? { icon: categoryData.icon, color: categoryData.color } : state.currentIcon
      }
...
export function useTodoManagement(enterAction) {
  ...
  const handleToggleStatus = useCallback((id, e) => {
    if (e) e.stopPropagation()
    const updatedTodo = toggleTodoStatus(id)
    if (updatedTodo) {
      dispatch({ type: 'UPDATE_TODO', payload: { id, updates: { completed: updatedTodo.completed } } })
      window.dispatchEvent(new Event('todo-updated'))
    }
  }, [])

  const handleFilterChange = useCallback((filter, label, icon, color, dueDate) => {
    dispatch({
      type: 'SET_FILTER',
  ...
  const handleCategoryChange = useCallback((category, label) => {
if (category === state.currentCategory) return
  ...
  const filteredTodos = useMemo(() => {
    return state.todos.filter(todo => {
      if (state.currentCategory !== null) {
        return (state.showCompleted || !todo.completed) && todo.categoryId === state.currentCategory
      }
      ...
      switch (state.currentFilter) {
        case 'today':
          return (state.showCompleted || !todo.completed) && todoDate.getTime() === today.getTime()
        case 'planned':
          return (state.showCompleted || !todo.completed) && todoDate.getTime() > today.getTime()
        case 'starred':
  ...
  }, [state.todos, state.currentCategory, state.currentFilter, state.showCompleted])
