        ...
        <div className='absolute inset-0 flex items-center justify-center pointer-events-none z-0'>
          <span
            className={cn(
              ...
              isToday ? 'text-primary rounded-full opacity-60' : ''
            )}
          >
            {format(day, 'd')}
          </span>
    ...
    <div className='h-full flex flex-col'>
      ...
      <div className='grid grid-cols-7 text-center text-sm font-medium flex-shrink-0'>
        {['周日', '周一', '周二', '周三', '周四', '周五', '周六'].map(
          (day, index) => (
            <div key={day} className='border-r border-b border-gray-200 py-1'>
              {day}
            </div>
          )
        )}
      ...
      {/* 日期网格 */}
      <div
        className='grid grid-cols-7 auto-rows-fr flex-1 min-h-0
