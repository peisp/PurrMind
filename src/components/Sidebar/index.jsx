    ...
    <aside className="w-72 h-full border-r border-gray-200 dark:border-gray-700/50 p-6 flex flex-col animate-slide-in">
      ...
      <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3">
            {stats.map(stat => (
              <StatCard label={stat.label} value={stat.value} color={stat.gradient} onClick={() => onFilterChange(stat.id)} />
            ))}
        </div>
        ...
        <div>
          ...
          <div className=" grid-cols-2 gap-3">
            {groups.map(group => (
              <StatCard label={group.label} value={group.value} color={group.gradient} onClick={() => onFilterChange(group.id)} />
            ))}
          </div>
        </div>
      </div>
